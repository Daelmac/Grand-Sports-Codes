...
import {addProduct,getProductByID,editProduct} from "../../../api/productApi";
...
const Product = ({userDetails}) => {
  ...
  const onProductSubmit = async (event) => {
    ...
    if (ProductValidation()) {
      if(id=='add'){
        const response = await dispatch(addProduct(userDetails,product));
        if(response){
      ...
      const response = await dispatch(editProduct(userDetails,product,id));
        if(response){
        if (response.status === "success") {
          // authenticateAdmin()
  ...
  const ProductValidation = () => {
    ...
    if (typeof product["discount"] !== "undefined") {
      alert("regr")
      if (!NumericRegX.test(product["discount"]) || parseInt(product["discount"])>100 ) {
        isValid = false;
    ...
    if (!product["description"] || product["description"].length < 12) {
      isValid = false;
      errors["descriptionErrMsg"] = "Product description must be at least 12 characters.";
    }
    setproductErrors(errors);
    return isValid;
  };    
       ...
       <Container>
          <Row>
            <Col lg={6} className="space-mb-mobile-only--50 mt-5">
              ...
              <MdEdit />
                <input type="file" name="image" accept="image/x-png,image/jpeg" onChange={onImageChange} />
                
              </div>
              <span className="error-text">
                          {productErrors.imageErrMsg}
                        </span>
            </Col>
                ...
                <form>
                  <Row>
                    ...
                    <Col lg={12} className="space-mb--50">
                      ...
                      <select class="custom-select" id="productCategory" name="category" value={product.category} onChange={handleProductDataChange}>
                        <option value="">Choose Product Category</option>
                        {
                          PRODUCR_CATEGORIES.map((category) =>
                    ...
                    <Col lg={12} className="space-mb--50">
                      ...
                      <select class="custom-select" id="productAvailable" name="isAvailable" value={product.isAvailable} onSelect={handleProductDataChange}>
                    ...
                    <button className="cancel-btn mr-2" onClick={onCancel}
                      ...
                      <button className="lezada-button lezada-button--medium" onClick={onProductSubmit}
                      >
